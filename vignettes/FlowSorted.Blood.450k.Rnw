...
et al. 2012 \cite{reinius:2012}, consisting of 60 samples, formatted as an \Rcode{RGset} object for
easy integration and normalization using existing Bioconductor packages (the code for creating the R
object from the raw \texttt{.idat} files is provided in the package as well at
